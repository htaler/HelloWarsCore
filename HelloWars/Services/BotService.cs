...
using HelloWars.Common.Helpers;
...
namespace Game.TankBlaster.Services
{
    internal class BotService
    {
        ...
        public bool AreMultipleBotsLeft => _field.AliveBots.Count >= 2;

        public Dictionary<ICompetitor, int> GetBotResults()
        {
            return _field.Bots.ToDictionary(bot => bot as ICompetitor, bot => bot.IsDead ? 0 : 1);
        }
        ...
        public void SetUpBots(IEnumerable<ICompetitor> competitors)
        {
            ...
            for (var i = 0; i < _field.Bots.Count; i++)
            {
                ...
                //TODO: Resolve Bot image, move to Presentation layer
        ...
        private BotBattlefieldInfo GetBotBattlefieldInfo(TankBlasterBot bot, int roundNumber, int turnNumber)
        {
            return new BotBattlefieldInfo
            {
                ...
                Missiles = _field.Missiles.ToList(),
